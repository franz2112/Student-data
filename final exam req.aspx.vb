...
Public Class final_exam_req
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        (d.OrderedQuantity*p.ProductStandardPrice) as TotalOrder
    ...
    Private Sub GetProductsByCustomer(ProductDescription As String)
        Dim connstring As String = "server=localhost;user id=root;database=Solmayor;allowuservariables=True"
        Dim conn As MySqlConnection = Nothing
        conn = New MySqlConnection(connstring)
        conn.Open()
        Dim query As String = "SELECT c.CustomerID, c.CustomerName, o.OrderID, p.ProductID, 
            d.OrderedQuantity as ProductOrders, p.ProductDescription as Description, p.ProductStandardPrice as UnitPrice,
            (d.OrderedQuantity*p.ProductStandardPrice) as TotalOrder
            from solmayor.customer_t c 
            INNER JOIN solmayor.order_t o on c.CustomerID = o.CustomerID 
            inner join solmayor.orderline_t d on o.OrderID = d.OrderID
            inner join solmayor.product_t p on p.ProductID = d.ProductID
            where p.ProductDescription = '" + ProductDescription + "' order by p.ProductDescription;"
        Dim da As New MySqlDataAdapter(query, conn)
        Dim ds As New DataSet()
        da.Fill(ds, "customer_t")
        GridView1.DataSource = ds
    ...
    Protected Sub Button4clear_Click(sender As Object, e As EventArgs) Handles Button4clear.Click
        Response.Redirect("final exam req.aspx")
    End Sub
End Class